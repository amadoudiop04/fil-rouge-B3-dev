...
import { type ValorantAgent } from '../data/agents';
import { C, DISP, MONO, UI, ORDER_STATUS_COLOR, DEFAULT_STATS, eur, fmtShortDate, fmtAgo, rankColor, useReveal } from './b3/theme';
import { ScreenHead, chipRow, kicker } from './b3/ui';
import { AgentsScreen } from './b3/AgentsScreen';
import { DiscordScreen } from './b3/DiscordScreen';
import { ReceiptModal, OrderDetailModal, type Receipt } from './b3/modals';
import { PublicProfileView } from './b3/PublicProfileView';
...
const B3App: React.FC<B3AppProps> = ({ user: authUser, cartCount, cartItems = [], onLogout, onRequireAuth, onAddToCart, onUpdateQty, onRemoveFromCart, onClearCart }) => {
  ...
  /* ── 03 AGENTS (extracted to ./b3/AgentsScreen) ──────────── */
  const renderAgentsScreen = () => (
    <AgentsScreen
      statsData={statsData} apiAgents={apiAgents}
      detailAgent={detailAgent} setDetailAgent={setDetailAgent}
      role={role} setRole={setRole} scanClip={scanClip} W={W}
    />
  );
  ...
  const screens: Record<Screen, () => React.ReactNode> = {
    home: renderHome, stats: renderStats, agents: renderAgentsScreen, players: renderPlayers,
    tournaments: renderTournaments, shop: renderShop, panier: renderPanier,
    discord: () => <DiscordScreen servers={discordServers} scanClip={scanClip} />,
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', width: '100%' }}>
      ...
      {receipt && (
        <ReceiptModal
          ...
          onContinue={() => { setReceipt(null); setScreen('shop'); }}
          onViewOrders={() => { setReceipt(null); setScreen('profile'); }}
        />
      )}
      ...
      <div style={{ position: 'relative', flex: 1, minHeight: 0 }}>
        ...
        <main ref={scrollRef} style={{ position: 'absolute', inset: 0, overflowY: 'auto', padding: '34px 40px 56px' }}>
          {viewProfile
            ? <PublicProfileView profile={viewProfile} onBack={() => setViewProfile(null)} scanClip={scanClip} />
            : screens[screen]()}
        </main>
      </div>
    </div>
